...
FMazePoint FRectangle::getBottomLeft() const {
	return FMazePoint(topLeft.x + width, topLeft.y + height);
}
...
FMazePoint FRectangle::getNearestPointFrom(const FRectangle& r) const
{
	...
	return FMazePoint(newXR2, newYR2);
}

FMazePoint FRectangle::getNearestPointTo(const FRectangle& r) const
{
	FMazePoint p = getNearestPointFrom(r);

	int newXR1 = clamp(p.x, topLeft.x, topLeft.x + width - 1);
	int newYR1 = clamp(p.y, topLeft.y, topLeft.y + height - 1);

	return FMazePoint(newXR1, newYR1);
}
uint64 FRectangle::getDistanceSquaredTo(FRectangle r2) const
{
	return FMazePoint::getDistanceSquared(getNearestPointTo(r2), getNearestPointFrom(r2));
}
...
bool FRectangle::containsPoint(const FMazePoint& p1) const
{
...
FRectangle FRectangle::getIntersectionRectangle(const FRectangle& r2) const {
	...
	FMazePoint p1 = getNearestPointFrom(r2);
	FMazePoint r2Down = r2.getBottomLeft();
